...
class User(Base):
    ...
    movements = relationship('Movement', back_populates='performed_by_user')  # type: list["Movement"]
    inventory_events = relationship('InventoryEvent', back_populates='performed_by_user')  # type: list["InventoryEvent"]
    maintenance_tasks = relationship('MaintenanceTask', back_populates='assigned_user')  # type: list["MaintenanceTask"]
    write_offs_disposed = relationship(
        'WriteOffReport', back_populates='disposed_by_user', foreign_keys='WriteOffReport.disposed_by'
